...
import { Calendar, Clock, MapPin, ChevronLeft, Video, Mic, Share2, Users } from 'lucide-react';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return generateDynamicMetadata({
    ...
    keywords: [event.category, 'Tech Event', 'Workshop', 'Vexel Systems', event.location],
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
       ...
       <div className="relative z-10 container mx-auto px-4 py-20 lg:py-32 flex flex-col gap-12">
          ...
          {/* Back Button */}
          <Link href="/events" className="inline-flex items-center gap-2 text-foreground/50 hover:text-primary transition-colors font-bold text-sm w-fit group">
             ...
             <div className="flex flex-col gap-10">
                <MotionWrapper type="slideUp">
                   ...
                   <div className="flex flex-wrap gap-6 text-sm font-bold text-foreground/70 mb-8 p-6 bg-card border border-foreground/5 rounded-2xl">
                      ...
                      <div className="flex items-center gap-2">
                         {event.type === 'Virtual' ? <Video size={18} className="text-primary"/> : <MapPin size={18} className="text-primary"/>}
                         {event.location}
                      </div>
                   </div>
                      ...
                      <div className="mt-12">
                         ...
                         <div className="space-y-4 not-prose">
                            {event.agenda.map((item, i) => (
                               <div key={i} className="flex gap-6 p-4 rounded-xl border border-foreground/5 bg-foreground/5 items-center">
                                  <div className="font-mono font-bold text-primary whitespace-nowrap">{item.time}</div>
                      ...
                      <div className="mt-12">
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 not-prose">
                            {event.speakers.map((s, i) => (
                               <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-card border border-foreground/5 shadow-sm">
                                  <div className="size-12 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold">
                                     {s.name.charAt(0)}
                                  </div>
                                  <div>
                                     <div className="font-bold">{s.name}</div>
                                     <div className="text-xs text-foreground/50 uppercase tracking-widest font-bold">{s.role}</div>
                                  </div>
                               </div>
                            ))}
                         </div>
                   ...
                   {event.status === 'Completed' ? (
                      <div className="flex flex-col gap-4 text-center">
                         ...
                         <p className="text-white/60 text-sm leading-relaxed">
                            This event has ended. Subscribe to our newsletter to get notified about future workshops and summaries.
                         </p>
                         <div className="py-4 mt-2 px-6 bg-white/5 rounded-xl border border-white/5">
                            <span className="text-xs font-bold uppercase tracking-widest text-white/40 block mb-1">Held On</span>
                      ...
                      <>
                        ...
                        <p className="text-white/60 text-sm mb-8">Limited seats available for this {event.type.toLowerCase()} event.</p>
                        
                        <form className="flex flex-col gap-4">
                            <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-widest opacity-50">Full Name</label>
                                <input type="text" placeholder="John Doe" className="w-full bg-white/10 border border-white/10 rounded-xl p-3 outline-none focus:bg-white/20 transition-all"/>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-widest opacity-50">Email Address</label>
                                <input type="email" placeholder="john@company.com" className="w-full bg-white/10 border border-white/10 rounded-xl p-3 outline-none focus:bg-white/20 transition-all"/>
                            </div>
                        ...
                        <p className="text-center text-[10px] text-white/30 mt-6 uppercase tracking-widest font-bold">Secure Vexel Ticketing</p>
                      </>
                   )}
                </div>
