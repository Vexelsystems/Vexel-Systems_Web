...
import { CheckCircle, Clock, Zap, Target, ArrowRight, MessageCircle, Phone, Globe, Rocket, Shield, Sparkles, Layers, Box } from "lucide-react";
        ...
        <div className="max-w-4xl mx-auto text-center">
            ...
            >
                <div className="text-center group">
                    <p className="text-[10px] font-black uppercase text-foreground/30 mb-2 tracking-widest">Estimated Delivery</p>
                ...
                <div className="text-center">
                    <p className="text-[10px] font-black uppercase text-foreground/30 mb-2 tracking-widest">Strategic Investment</p>
                    ...
                    {pkg.monthlyCost && (
                         <div className="text-xs font-bold text-foreground/40 mt-1 uppercase tracking-wider">
                            + {currency === 'LKR' ? pkg.monthlyCost.lkr : pkg.monthlyCost.usd} Maintenance
                         </div>
                    )}
                     <div className="flex justify-center gap-1.5 mt-4">
                        <button onClick={() => setCurrency('LKR')} className={`text-[10px] font-extrabold px-3 py-1 rounded-full transition-all ${currency === 'LKR' ? 'bg-primary text-white shadow-lg' : 'bg-black/5 dark:bg-white/5 text-foreground/40 hover:text-foreground'}`}>LKR</button>
                ...
                >
                    ...
                    <div className="relative z-10">
                        <div className="size-16 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mb-8 group-hover:rotate-12 transition-transform shadow-inner">
                            {idx === 0 ? <Zap size={32} /> : idx === 1 ? <Rocket size={32} /> : <Layers size={32} />}
                        </div>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        {[
                            ...
                            <li key={i} className="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-foreground/40 font-medium">{t.desc}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
                ...
                >
                    ...
                    <ul className="space-y-4">
                        {[
                            { name: "SSL Encryption", desc: "End-to-end data protection for all users." },
                            { name: "WAF Protection", desc: "Custom firewall to block SQL injection & XSS." },
                            { name: "Secure Auth", desc: "Multi-factor ready authentication systems." },
                            { name: "Daily Backups", desc: "Continuous state snapshots for disaster recovery." }
                        ].map((t, i) => (
                            <li key={i} className="flex gap-4">
                ...
                >
                    ...
                    <ul className="space-y-4">
                        {[
                            { name: "Semantic HTML", desc: "Proper h1-h6 hierarchy for screen readers & bots." },
                            { name: "JSON-LD Schema", desc: "Rich snippets to show prices/rating on Google." },
                            { name: "Meta Architecture", desc: "Dynamic Open Graph & Twitter Card setup." },
                            { name: "Performance SEO", desc: "Sub-second LCP scores for higher rankings." }
                        ].map((t, i) => (
                            <li key={i} className="flex gap-4">
      ...
      <section className="max-w-4xl mx-auto px-6 py-32">
        ...
        >
            <h2 className="text-3xl font-black mb-4 flex items-center justify-center gap-3">
                <span className="p-2 bg-emerald-500/10 rounded-xl text-emerald-600"><CheckCircle size={24} /></span>
                Master Deliverables
            </h2>
      ...
      <section id="quote" className="max-w-4xl mx-auto px-6 animate-in fade-in slide-in-from-bottom-12 duration-1000">
        <div className="relative overflow-hidden rounded-[4rem] bg-gray-900 text-white shadow-2xl">
            ...
            <div className="relative z-10 p-12 md:p-24 text-center">
                <h2 className="text-4xl md:text-6xl font-black mb-8 leading-tight">Start Your <span className="text-primary">Journey.</span></h2>
                <p className="text-xl opacity-70 mb-12 max-w-xl mx-auto font-medium">
                    Enter your brand name below and our AI will prepare a tailored draft for our first consultation.
                </p>
                ...
                <div className="mt-12 flex flex-wrap justify-center gap-8">
                    <Link href={`tel:${companyDetails.contact.phone}`} className="flex items-center gap-3 font-black text-white/60 hover:text-white transition-colors">
                        <Phone size={20} className="text-primary" /> Direct Call
                    </Link>
                    <Link href={`https://wa.me/${companyDetails.contact.whatsapp.replace('+', '')}`} className="flex items-center gap-3 font-black text-white/60 hover:text-white transition-colors">
