    ...
    <div className="container w-[90%] md:w-[80%] mx-auto max-w-7xl flex flex-col gap-16 pb-20">
      ...
      <section className="flex flex-col gap-12">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <span className="text-foreground/40 font-bold uppercase text-xs tracking-widest mr-4">Filter By Category:</span>
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all border ${
                ...
                : 'bg-black/5 dark:bg-white/5 text-foreground/60 border-transparent hover:border-black/10 dark:hover:border-white/10'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPosts.map((post, index) => (
            ...
            >
              <Link 
                href={`/blog/${post.slug}`}
                className="group block h-full bg-white dark:bg-white/5 rounded-[32px] border border-black/5 dark:border-white/5 overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-2"
              >
                {/* Image Container */}
                ...
                <div className="p-8 flex flex-col h-[calc(100%-16rem)]">
                  <div className="flex items-center gap-4 text-xs font-bold text-foreground/40 mb-4 uppercase tracking-widest">
                  ...
                  <p className="text-foreground/60 leading-relaxed mb-8 flex-1 line-clamp-3">
                    {post.excerpt}
                  </p>
