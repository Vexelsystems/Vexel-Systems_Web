...
const INTERNATIONAL_PACKAGES: Package[] = [
  {
    ...
    price: "$300 – $500",
    features: [
      "Basic multi-page site (up to 5 pages)",
      "Responsive Design",
      "Contact Form",
      "Basic SEO Setup",
    ],
...
const MAINTENANCE_PACKAGES_LKR = [
  {
    ...
    features: ["Reliable Hosting", "Small Content Edits", "Uptime Monitoring"],
  },
  {
    name: "Pro Care",
...
export default function PricingClient() {
  ...
  const [comparingCategory, setComparingCategory] =
      ...
      <section className="relative w-full mb-12 text-center bg-transparent">
        <MotionWrapper type="slideUp" duration={1.2}>
          <div className="container w-[90%] md:w-[80%] mx-auto max-w-[1920px] relative">
            {/* Currency Toggle */}
            <div className="absolute top-0 right-0 flex bg-card/40 dark:bg-nav-bg/40 backdrop-blur-md border border-primary/20 rounded-full p-1 shadow-sm">
              <button
            ...
            <p className="text-xl text-foreground/80 max-w-2xl mx-auto mb-10">
              ...
              {currency === "LKR" ? "Sri Lankan" : "International"} businesses.
              {currency === "USD" && (
            ...
            <div className="flex flex-wrap justify-center gap-3 max-w-5xl mx-auto mb-12">
              {SUMMARY_DATA.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2 px-4 py-2 bg-card/50 dark:bg-nav-bg/10 backdrop-blur-md rounded-full border border-primary/10 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
                >
                  <span className="text-sm font-medium text-foreground/80">
                    {item.type}
                  </span>
                  <span className="text-sm font-bold text-primary">
                    starts {currency === "LKR" ? item.priceLKR : item.priceUSD}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-linear-to-br from-indigo-900 via-purple-900 to-primary/50 rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="text-center mb-12 relative z-10">
              ...
              <p className="text-white/80 max-w-2xl mx-auto">
                All-in-one solutions tailored for our international partners.
              </p>
            </div>
            ...
            <SnapCarousel scrollContainerClassName="md:grid md:grid-cols-3 gap-6 relative z-10 pb-8 md:pb-0 scrollbar-hide -mx-8 px-8 md:mx-0 md:px-0">
              {INTERNATIONAL_PACKAGES.map((pkg, idx) => (
                <div
                  key={idx}
                  className="min-w-[85vw] md:min-w-0 snap-center bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 hover:bg-white/20 transition-all duration-300 hover:shadow-xl hover:-translate-y-2"
                >
                  <h3 className="text-2xl font-bold mb-2">{pkg.name}</h3>
                  <div className="text-4xl font-black mb-6 text-transparent bg-clip-text bg-linear-to-r from-blue-200 to-white">
                    {pkg.price}
                  </div>
                  ...
                  <Link
                    href="/contact"
                    className="block w-full text-center bg-white text-primary font-bold py-3 rounded-xl hover:bg-blue-50 transition-colors shadow-lg"
                  >
                    Get Started
                  </Link>
                </div>
      ...
      <section className="relative py-32 mt-32 overflow-hidden bg-transparent">
        ...
        <div className="container w-[90%] md:w-[80%] mx-auto max-w-[1920px] relative z-10 text-white">
          ...
          <SnapCarousel scrollContainerClassName="md:grid md:grid-cols-2 gap-8 max-w-5xl mx-auto pb-8 md:pb-0 scrollbar-hide -mx-6 px-6 md:mx-auto md:px-0">
            {(currency === "LKR"
              ? MAINTENANCE_PACKAGES_LKR
              : MAINTENANCE_PACKAGES_USD
            ).map((pkg, idx) => (
              <div
                key={idx}
                className={`min-w-[85vw] md:min-w-0 snap-center relative group p-1 rounded-3xl bg-linear-to-b ${idx === 0 ? "from-blue-500/50 to-blue-900/10" : "from-purple-500/50 to-purple-900/10"} hover:scale-[1.02] transition-transform duration-500`}
              >
                <div className="h-full bg-gray-950/90 backdrop-blur-xl rounded-[22px] p-8 md:p-10 flex flex-col md:flex-row items-center gap-8 border border-white/5">
                  <div
                    className={`p-6 rounded-2xl shrink-0 ${idx === 0 ? "bg-blue-500/20 text-blue-400 shadow-lg shadow-blue-500/20" : "bg-purple-500/20 text-purple-400 shadow-lg shadow-purple-500/20"}`}
                  >
                    {idx === 0 ? (
                      <Server size={40} />
                    ) : (
                  ...
                  <div className="text-center md:text-left grow">
                    ...
                    <ul className="space-y-3">
                      {pkg.features.map((f, i) => (
                        <li
                          key={i}
                          className="text-sm font-medium text-white/70 flex items-center justify-center md:justify-start gap-3"
                        >
                          <CheckCircle
                            size={16}
                            className={
                  ...
                  <Link
                    href="/contact"
                    className="hidden lg:flex size-14 rounded-full bg-white/10 hover:bg-white text-white hover:text-gray-900 items-center justify-center transition-all"
                  >
                    <ArrowRight size={24} />
                  </Link>
                </div>
